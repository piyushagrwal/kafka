...
if hash docker 2>/dev/null; then
    # for docker lovers
    docker build . -t github-connector/kafka-connect-source-github:1.0
    docker run --net=host --rm -t \
           -v $(pwd)/offsets:/kafka-custom-connector/offsets \
           github-connector/kafka-connect-source-github:1.0
